...
/* Clones the current process as `name`. Returns the new process's thread id,
...
process_fork (const char *name, struct intr_frame *if_ UNUSED) {
	...
	sema_init (&info->fork_wait, 0);
	tid_t temp = thread_create (name, PRI_DEFAULT, __do_fork, info);
	sema_down (&info->fork_wait);
	free (info);
	return temp;
}
...
duplicate_pte (uint64_t *pte, void *va, void *aux) {
	 ...
	 *    TODO: check whether parent's page is writable or not (set
	 * WRITABLE
...
__do_fork (void *aux) {
	...
	struct semaphore *fork_sema = &info->fork_wait;
	struct thread *parent_thread = info->parent_thread;
	struct thread *current = thread_current ();
	struct intr_frame if_;
	/* TODO: somehow pass the parent_if. (i.e. process_fork()'s if_) */
	struct intr_frame *parent_if = info->f_;
	bool succ = true;
	...
	current->my_status = create_child_status (parent_thread);
	current->my_status->tid = current->tid;
	for (int i = 3; i < FD_SIZE; i++) {
		if (parent_thread->fd[i] != 0) {
			current->fd[i] = file_duplicate (parent_thread->fd[i]);
		}
	}
	process_init ();
