...
/* Creates an gate that invokes FUNCTION.
   ...
   DPL==0 to be invoked.
   이 게이트는 DPL이라는 디스크립터 권한 수준을 가진다. 프로세서가
   DPL 또는 그보다 숫자가 낮은 링에 있을 때 의도적으로 호출할 수
   있다는 뜻이다. 실제로 DPL==3이면 사용자 모드에서 게이트를 호출할 수
   있고, DPL==0이면 그런 호출을 막는다. 사용자 모드에서 발생한 fault와
   exception은 여전히 DPL==0 게이트를 호출하게 된다.
   ...
   차이는 interrupt gate에 진입하면 인터럽트가 비활성화되지만, trap gate에
   진입할 때는 그렇지 않다는 점이다. 자세한 내용은 [IA32-v3a] 5.12.1.2
...
/* The Interrupt Descriptor Table (IDT).  The format is fixed by
   ...
   인터럽트 디스크립터 테이블(IDT). 형식은 CPU에 의해 고정되어 있다.
...
/* External interrupts are those generated by devices outside the
   ...
   interrupt returns.
   외부 인터럽트는 타이머처럼 CPU 바깥의 장치가 생성하는 인터럽트다.
   외부 인터럽트는 인터럽트가 꺼진 상태에서 실행되므로 중첩되지 않고,
   선점되지도 않는다. 외부 인터럽트 핸들러는 sleep할 수 없지만,
   인터럽트에서 돌아가기 직전에 새 프로세스를 스케줄하도록 요청하기 위해
   intr_yield_on_return()을 호출할 수 있다. */
...
intr_get_level (void) {
	...
	/* Push the flags register on the processor stack, then pop the
	   ...
	   Interrupts".
	   프로세서 스택에 flags 레지스터를 push한 뒤, 그 값을 스택에서 꺼내
	   `flags`에 저장한다. [IA32-v2b] "PUSHF", "POP" 및 [IA32-v3a]
	   5.8.1 "Masking Maskable Hardware Interrupts"를 참고한다. */
...
/* Enables or disables interrupts as specified by LEVEL and
   ...
   LEVEL에 따라 인터럽트를 활성화하거나 비활성화하고, 이전 인터럽트
   상태를 반환한다. */
enum intr_level
...
intr_enable (void) {
	...
	/* Enable interrupts by setting the interrupt flag.
	   ...
	   Hardware Interrupts".
	   interrupt flag를 설정해 인터럽트를 활성화한다. [IA32-v2b] "STI"와
...
intr_disable (void) {
	...
	/* Disable interrupts by clearing the interrupt flag.
	   ...
	   Hardware Interrupts".
	   interrupt flag를 지워 인터럽트를 비활성화한다. [IA32-v2b] "CLI"와
	   [IA32-v3a] 5.8.1 "Masking Maskable Hardware Interrupts"를 참고한다. */
...
/* Registers interrupt VEC_NO to invoke HANDLER with descriptor
   ...
   인터럽트 VEC_NO가 디스크립터 권한 수준 DPL로 HANDLER를 호출하도록
   등록한다. 디버깅을 위해 인터럽트에 NAME이라는 이름을 붙인다.
   인터럽트 핸들러는 인터럽트 상태가 LEVEL로 설정된 상태에서 호출된다. */
static void
...
/* Registers external interrupt VEC_NO to invoke HANDLER, which
   ...
   외부 인터럽트 VEC_NO가 HANDLER를 호출하도록 등록하고, 디버깅을 위해
   NAME이라는 이름을 붙인다. 핸들러는 인터럽트가 비활성화된 상태에서
   실행된다. */
void
...
/* Registers internal interrupt VEC_NO to invoke HANDLER, which
   ...
   discussion.
   내부 인터럽트 VEC_NO가 HANDLER를 호출하도록 등록하고, 디버깅을 위해
   NAME이라는 이름을 붙인다. 인터럽트 핸들러는 인터럽트 상태 LEVEL로
   호출된다.

   핸들러는 디스크립터 권한 수준 DPL을 가진다. 프로세서가 DPL 또는
   그보다 숫자가 낮은 링에 있을 때 의도적으로 호출할 수 있다는 뜻이다.
   실제로 DPL==3이면 사용자 모드에서 인터럽트를 호출할 수 있고,
   DPL==0이면 그런 호출을 막는다. 사용자 모드에서 발생한 fault와
   exception은 여전히 DPL==0 인터럽트를 호출하게 된다. 자세한 내용은
   [IA32-v3a] 4.5 "Privilege Levels"와 4.8.1.1 "Accessing Nonconforming
   Code Segments"를 참고한다. */
void
...
/* Returns true during processing of an external interrupt
   ...
   외부 인터럽트를 처리 중이면 true를 반환하고, 그 외에는 false를 반환한다. */
bool
intr_context (void) {
...
/* During processing of an external interrupt, directs the
   ...
   time.
   외부 인터럽트를 처리하는 동안, 인터럽트에서 반환하기 직전에 새 프로세스에
   yield하도록 인터럽트 핸들러에 지시한다. 다른 시점에는 호출할 수 없다. */
void
...
/* Every PC has two 8259A Programmable Interrupt Controller (PIC)
   ...
   모든 PC에는 두 개의 8259A Programmable Interrupt Controller(PIC) 칩이
   있다. 하나는 0x20과 0x21 포트로 접근하는 "master"이고, 다른 하나는
   master의 IRQ 2 라인에 연결된 "slave"로 0xa0과 0xa1 포트로 접근한다.
   0x20 포트 접근은 A0 라인을 0으로, 0x21 접근은 A1 라인을 1로 설정한다.
   slave PIC도 비슷하다.

   기본적으로 PIC가 전달하는 인터럽트 0...15는 인터럽트 벡터 0...15로
   전달된다. 하지만 이 벡터들은 CPU trap과 exception에도 사용된다.
   그래서 PIC를 재프로그래밍하여 인터럽트 0...15가 대신 인터럽트 벡터
   32...47(0x20...0x2f)로 전달되도록 한다. */
...
pic_init (void) {
	...
	outb (0xa1, 0x28); /* ICW2: line IR0...7 -> irq 0x28...0x2f.
	                      ICW2: IR0...7 라인을 irq 0x28...0x2f에 매핑한다. */
...
/* Sends an end-of-interrupt signal to the PIC for the given IRQ.
   ...
   주어진 IRQ에 대해 PIC에 end-of-interrupt 신호를 보낸다. IRQ를 확인해
   주지 않으면 다시 전달되지 않으므로 중요하다. */
static void
pic_end_of_interrupt (int irq) {
	ASSERT (irq >= 0x20 && irq < 0x30);

	/* Acknowledge master PIC.
	   master PIC에 인터럽트 처리가 끝났음을 알린다. */
	outb (0x20, 0x20);

	/* Acknowledge slave PIC if this is a slave interrupt.
	   slave 인터럽트라면 slave PIC에도 인터럽트 처리가 끝났음을 알린다. */
	if (irq >= 0x28)
		outb (0xa0, 0x20);
}
/* Interrupt handlers.
...
/* Handler for all interrupts, faults, and exceptions.  This
   ...
   모든 인터럽트, fault, exception을 처리하는 핸들러다. 이 함수는
   intr-stubs.S의 어셈블리 인터럽트 stub에서 호출된다. FRAME은 인터럽트와
   인터럽트된 스레드의 레지스터를 설명한다. */
void
intr_handler (struct intr_frame *frame) {
	...
	/* External interrupts are special.
	   ...
	   있어야 하고, PIC에 처리를 확인해 주어야 한다(아래 참고).
	...
	if (handler != NULL)
		...
		/* There is no handler, but this interrupt can trigger
		   ...
		   condition.  Ignore it.
		   핸들러는 없지만, 이 인터럽트는 하드웨어 결함이나 하드웨어 경쟁
		   상태 때문에 가짜로 발생할 수 있다. 무시한다. */
	} else {
		/* No handler and not spurious.  Invoke the unexpected
		   interrupt handler.
		   핸들러도 없고 가짜 인터럽트도 아니다. 예기치 않은 인터럽트
		   핸들러를 호출한다. */
...
intr_dump_frame (const struct intr_frame *f) {
	/* CR2 is the linear address of the last page fault.
	   ...
	   (#PF)".
	   CR2는 마지막 page fault의 선형 주소다. [IA32-v2a] "MOV--Move
	   to/from Control Registers"와 [IA32-v3a] 5.14 "Interrupt 14--Page
	   Fault Exception (#PF)"를 참고한다. */
	uint64_t cr2 = rcr2();
