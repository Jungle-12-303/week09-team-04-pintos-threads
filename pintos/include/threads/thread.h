 ...
 * 구조체가 너무 커지거나 커널 스택을 너무 많이 쓰면 magic 값이 깨져 오류를
 * 발견할 수 있다. */
 ...
 * READY 스레드와 BLOCKED 스레드는 동시에 같은 리스트에 들어가지 않기 때문에
 * 공유할 수 있다. */
struct thread {
	/* Owned by thread.c. */
	...
	/* ready list 또는 동기화 객체 대기 목록에 연결될 때 사용하는 공용 리스트
	 * 요소. */
	struct list_elem elem; /* List element. */

	/* for sleeping list */
	/* timer_sleep()으로 잠든 스레드를 sleeping_list에 연결하기 위한 리스트
	 * 요소. */
...
/* 리스트 정렬에 사용하는 비교 함수들. */
bool thread_wakeup_tick_less (const struct list_elem *a,
